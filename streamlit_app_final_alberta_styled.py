...
with col3:
    if step < len(steps) - 1:
        ...
        if st.button("🎯 Estimate My Funding"):
            ...
            st.markdown("<h2 style='color:#003366; font-family:Helvetica, sans-serif;'>🎓 Estimated Funding Summary</h2>", unsafe_allow_html=True)
            ...
            for name, amount in grants.items():
                if amount > 0:
                    st.markdown(f"<p style='margin-left:20px;'>✔️ <strong>{name}</strong>: {format_currency(amount)}</p>", unsafe_allow_html=True)

            st.markdown("<h4 style='color:#003366;'>🏦 Loans</h4>", unsafe_allow_html=True)
            st.markdown(f"<p style='margin-left:20px;'>✔️ <strong>Canada Loan:</strong> {format_currency(result['Canada Loan'])}</p>", unsafe_allow_html=True)
            st.markdown(f"<p style='margin-left:20px;'>✔️ <strong>Alberta Loan:</strong> {format_currency(result['Alberta Loan'])}</p>", unsafe_allow_html=True)
            ...
            st.markdown("###")
            if st.button("🔄 Start Over", key="start_over_btn_2779"):
