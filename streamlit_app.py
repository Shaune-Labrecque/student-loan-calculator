...
def main():
    ...
    # Handle disability status change - remove this problematic logic
    ...
    if st.session_state.results:
        ...
        with col3:
            current_steps = get_steps(st.session_state.form_data['has_disability'])

            if st.session_state.current_step < len(current_steps) - 1:
                if st.button("Next →"):
                    if validate_current_step():
                        # ✅ Handle disability toggle dynamically
                        if current_steps[st.session_state.current_step]['id'] == "disability" and st.session_state.form_data['has_disability']:
                            # Add disability_costs step *only if* it's not already included
                            if not any(s['id'] == 'disability_costs' for s in current_steps):
                                st.session_state.current_step += 1
                                st.rerun()
                        else:
                            st.session_state.current_step += 1
