...
with col3:
    if step < len(steps) - 1:
        ...
        if st.button("🎯 Estimate My Funding"):
            ...
            # Section: Grants
            st.markdown("## 💰 Grants")
            st.markdown("<div style='background-color:#F3F6FA; padding: 20px; border-radius: 10px;'>", unsafe_allow_html=True)
            grants = {
            ...
            for name, amount in grants.items():
                if amount > 0:
                    st.markdown(f"<p style='font-size:1.1rem;'><strong>{name}</strong>: {format_currency(amount)}</p>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)

            # Section: Loans
            st.markdown("## 🏦 Loans")
            st.markdown("<div style='background-color:#FFF6EE; padding: 20px; border-radius: 10px;'>", unsafe_allow_html=True)
            st.markdown(f"<p style='font-size:1.1rem;'><strong>Canada Loan</strong>: {format_currency(result['Canada Loan'])}</p>", unsafe_allow_html=True)
            st.markdown(f"<p style='font-size:1.1rem;'><strong>Alberta Loan</strong>: {format_currency(result['Alberta Loan'])}</p>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)

            # Section: Total
            st.markdown("## 🎯 Total Estimated Funding")
            st.markdown(f"<div style='text-align:center; font-size:2rem; color:#0055A4; font-weight:bold; padding: 10px 0;'>{format_currency(result['Total Funding'])}</div>", unsafe_allow_html=True)

            st.markdown("###")
            if st.button("🔄 Start Over", key="start_over_card_layout"):
