...
with col3:
    if step < len(steps) - 1:
        ...
        if st.button("🎯 Estimate My Funding"):
            ...
            st.markdown("## 🎓 Estimated Funding Summary")
            ...
            for name, amount in grants.items():
                if amount > 0:
                    st.markdown(f"- **{name}:** {format_currency(amount)}")

            st.markdown("### 🏦 Loans")
            st.markdown(f"- **Canada Loan:** {format_currency(result['Canada Loan'])}")
            st.markdown(f"- **Alberta Loan:** {format_currency(result['Alberta Loan'])}")
